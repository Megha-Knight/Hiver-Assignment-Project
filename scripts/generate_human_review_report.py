"""Generates results/phase7/phase7d_human_review_report.md covering all 14 required sections."""
...
with open(PROJECT_ROOT / "results" / "phase7" / "phase7d_human_agreement.json", "r", encoding="utf-8") as f:
    agr_data = json.load(f)

per_dim = agr_data["per_dimension_agreement"]

rel_h_mean = per_dim['relevance']['human_mean']
rel_j_mean = per_dim['relevance']['judge_mean']
rel_mad = per_dim['relevance']['mean_absolute_difference']
rel_exact = per_dim['relevance']['exact_agreement_rate'] * 100
rel_adj = per_dim['relevance']['adjacent_agreement_rate'] * 100
rel_qwk = per_dim['relevance']['quadratic_weighted_kappa']
rel_rho = per_dim['relevance']['spearman_rho']

help_h_mean = per_dim['helpfulness']['human_mean']
help_j_mean = per_dim['helpfulness']['judge_mean']
help_mad = per_dim['helpfulness']['mean_absolute_difference']
help_exact = per_dim['helpfulness']['exact_agreement_rate'] * 100
help_adj = per_dim['helpfulness']['adjacent_agreement_rate'] * 100
help_qwk = per_dim['helpfulness']['quadratic_weighted_kappa']
help_rho = per_dim['helpfulness']['spearman_rho']

gro_h_mean = per_dim['groundedness']['human_mean']
gro_j_mean = per_dim['groundedness']['judge_mean']
gro_mad = per_dim['groundedness']['mean_absolute_difference']
gro_exact = per_dim['groundedness']['exact_agreement_rate'] * 100
gro_adj = per_dim['groundedness']['adjacent_agreement_rate'] * 100
gro_qwk = per_dim['groundedness']['quadratic_weighted_kappa']
gro_rho = per_dim['groundedness']['spearman_rho']

act_h_mean = per_dim['action_appropriateness']['human_mean']
act_j_mean = per_dim['action_appropriateness']['judge_mean']
act_mad = per_dim['action_appropriateness']['mean_absolute_difference']
act_exact = per_dim['action_appropriateness']['exact_agreement_rate'] * 100
act_adj = per_dim['action_appropriateness']['adjacent_agreement_rate'] * 100
act_qwk = per_dim['action_appropriateness']['quadratic_weighted_kappa']
act_rho = per_dim['action_appropriateness']['spearman_rho']

safe_h_mean = per_dim['safety']['human_mean']
safe_j_mean = per_dim['safety']['judge_mean']
safe_mad = per_dim['safety']['mean_absolute_difference']
safe_exact = per_dim['safety']['exact_agreement_rate'] * 100
safe_adj = per_dim['safety']['adjacent_agreement_rate'] * 100
safe_qwk = per_dim['safety']['quadratic_weighted_kappa']
safe_rho = per_dim['safety']['spearman_rho']

tone_h_mean = per_dim['communication_quality']['human_mean']
tone_j_mean = per_dim['communication_quality']['judge_mean']
tone_mad = per_dim['communication_quality']['mean_absolute_difference']
tone_exact = per_dim['communication_quality']['exact_agreement_rate'] * 100
tone_adj = per_dim['communication_quality']['adjacent_agreement_rate'] * 100
tone_qwk = per_dim['communication_quality']['quadratic_weighted_kappa']
tone_rho = per_dim['communication_quality']['spearman_rho']

ov_h_mean = agr_data['overall_human_mean']
ov_j_mean = agr_data['overall_judge_mean']
ov_mad = agr_data['overall_mean_absolute_difference']
ov_exact = agr_data['overall_exact_agreement_rate'] * 100
ov_adj = agr_data['overall_adjacent_agreement_rate'] * 100

sections = []

sections.append("""# Phase 7D: Human Response-Quality Validation & Final Evaluation-Hardening Report
...
table_md = f"""| Quality Dimension | Human Mean | Judge Mean | Mean Absolute Difference (MAD) | Exact Agreement (P0) | Adjacent Agreement (P±1) | Quadratic Weighted Kappa (QWK) | Spearman Rank (Rho) |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **Relevance** | {rel_h_mean} | {rel_j_mean} | **{rel_mad}** | {rel_exact:.1f}% | {rel_adj:.1f}% | {rel_qwk:.4f} | **{rel_rho:.4f}** |
| **Helpfulness** | {help_h_mean} | {help_j_mean} | **{help_mad}** | {help_exact:.1f}% | {help_adj:.1f}% | {help_qwk:.4f}* | {help_rho:.4f}* |
| **Groundedness** | {gro_h_mean} | {gro_j_mean} | **{gro_mad}** | {gro_exact:.1f}% | **{gro_adj:.1f}%** | {gro_qwk:.4f}* | {gro_rho:.4f}* |
| **Action Appropriateness** | {act_h_mean} | {act_j_mean} | **{act_mad}** | {act_exact:.1f}% | {act_adj:.1f}% | {act_qwk:.4f}* | {act_rho:.4f}* |
| **Safety** | {safe_h_mean} | {safe_j_mean} | **{safe_mad}** | **{safe_exact:.1f}%** | **{safe_adj:.1f}%** | **{safe_qwk:.4f}** | **{safe_rho:.4f}** |
| **Communication Tone** | {tone_h_mean} | {tone_j_mean} | **{tone_mad}** | {tone_exact:.1f}% | **{tone_adj:.1f}%** | {tone_qwk:.4f}* | {tone_rho:.4f}* |
| **OVERALL COMPOSITE** | **{ov_h_mean}** | **{ov_j_mean}** | **{ov_mad}** | **{ov_exact:.1f}%** | **{ov_adj:.1f}%** | -- | -- |
"""
...
report_full = "\n".join(sections)
out_path = PROJECT_ROOT / "results" / "phase7" / "phase7d_human_review_report.md"
with open(out_path, "w", encoding="utf-8") as f:
    f.write(report_full)

print(f"Successfully generated {out_path} ({len(report_full)} chars)")
